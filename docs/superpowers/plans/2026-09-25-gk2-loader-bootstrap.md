...
namespace GK2ModInstaller.Loader
{
    ...
    public static class Entry
    {
        public static void Run(string bepInExRoot, string gameRoot, string workshopRoot, string acfPath, string source)
        {
            ...
            // Язык интерфейса: конфиг BepInEx\config\GK2_WorkshopLoader.txt (auto|en|ru) + системный язык.
            // Без этой строки русские пользователи получат английские диалоги (проверено ревью).
...
## ?????????? ?? E2E (25.09) � ?????? 7-9 (????????? ?? Task 6 �???????????? ? ??????�)

???????: ??. �10 ?????. ????????? ?? ????? ?????????/??????? ??? ??????????? `Managed\*.dll` game-folder ????.

### Task 7: ???????? DLL game-folder ????? ?????? ??????????? (B)

**Files:** ??????? `src/GK2ModInstaller.Core/GameFolderDllLoader.cs`; ???????? `GameFolderInstaller.cs` (?????????? ?????? ??-DLL), `WorkshopLoader.cs` (??????? ?????????), `GameFolderInstallerTests.cs` (+ ????? ?????).
- `GameFolderDllLoader.Load(IEnumerable<string> dllPaths, Action<string> log) ? int` � ?????? ?????? ???????????? DLL ????? `Assembly.Load(File.ReadAllBytes(path))`, ????????? ????? ?? ????????? (HashSet ?????), ?????? ????????, ?? ???????.
- `GameFolderInstaller.Install` � ?????????? `*.dll` ? `Managed` (? ?????? DLL ? ??????? ?????): ?? ?????? � ????????, ?? ?????; ?????/???????? ?? ??? ?? ?????.
- `WorkshopLoader`: ?? `Approve`/`Known` ??? GameFolder-???? � ????????? ??? DLL ?? `SourceDir` ? ??????????? ?????? ??-DLL; ?? `Deny`/`Removed` � DLL ?? ?????????? (??????), ?????? ??????.
- ?????: ????????????? DLL/??-DLL; `Install` ?? ???????? DLL; `Load` ???????????? ? ?????? ? ????? ?????? (???????? ?? temp-??????).

### Task 8: Pending-???????? ? ???????? legacy-????? (A)

**Files:** ??????? `src/GK2ModInstaller.Core/PendingGameActions.cs`; ???????? `WorkshopLoader.cs` (?????? pending ??? ????/legacy), `BepInExInstaller.cs` (`ApplyPendingGameActions(gameDir, log)`), ?????.
- ?????? `BepInEx\config\GK2_WorkshopLoader.pendinggame.txt`: ?????? `delete|<????????????? ????>` / `restore|<rel>|<id>`; ??????????? `#`.
- ????????? ??? ????/??????????? legacy-????? (id ???? ? ????? backup-??????????) ?????????? ????????.
- `BepInExInstaller.ApplyPendingGameActions` ??? ???????? ????: ???????/??????????????? ????? ?? ??????, ?????? ??????????? ??????, ???????? (???????). ???????? ?? Install ? ?? CLI (`--install`).
- ?????: ???????/?????? pending, ?????????? delete/restore ?? temp-??????, ???????????????.

### Task 9: ?? ?????????? ?????????? ???, ???? ????????? ???????????

**Files:** ???????? `GameFolderInstaller.cs` (?????????? ????? ??????: `Install(...)`/`Restore(...)` ? out/int), `WorkshopLoader.cs` (??? ???????? ????????? ??????? trust-??? ? `Note` �?????????? ????????�), ?????.
- ????????: ??????, ??? ??????? ???? ????? (? ????? � ?? ????? ????? ??????????), ?? ?????? ??? ? trust ? ????????; ????????? ?????? ????? ????? `Update`.
