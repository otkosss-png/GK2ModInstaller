...
## 10. Дополнение по E2E (25.09): game-folder моды нельзя менять изнутри игры

Проверено на живом `Recipe Pin` (3807460866):
- **первичная установка** game-folder мода из прелоадера работает (файлов ещё нет — блокировать нечего);
- **обновление/удаление уже установленного** — НЕТ: `Managed\0Harmony.dll`, `GK2RecipePin.dll`, `GK2RecipePin.Core.dll` залочены процессом уже на самом раннем хуке (`UnityEngine.CoreModule.dll`, как у самого BepInEx). Загрузчик ведёт себя fail-closed (бэкап сохранён, файлы не тронуты), но апдейт не применяется;
- **баг учёта**: если апдейт одобрен, а копирование не удалось, trust всё равно записывает новый хеш → повтор не произойдёт. Нужно сохранять прежний хеш, чтобы следующий запуск снова увидел `Update`.

**Решение (вариант C, выбрано пользователем):**
- **B** — DLL game-folder мода **не копировать** в папку игры, а грузить в процесс из айтема (`Assembly.Load(File.ReadAllBytes(...))`, `0Harmony` резолвится из `BepInEx\core`); копировать по-прежнему только **не-DLL** файлы (`Languages`, данные) — они не лочатся. Тогда обновления/откат не зависят от локов.
- **A** — то, что нельзя сделать в игре (удаление legacy-копий DLL из `Managed`, откат при отписке), загрузчик пишет в `BepInEx\config\GK2_WorkshopLoader.pendinggame.txt`, а `GK2 Mod Installer` применяет это при закрытой игре.
- Плюс: починить учёт trust при неудачной установке (см. выше).

Риски: не все такие DLL умеют грузиться вне `Managed` (проверять на живых модах); у существующих пользователей остаются старые копии в `Managed` — нужна разовая миграция через A.
