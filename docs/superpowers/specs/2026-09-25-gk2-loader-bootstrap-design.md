...
## 10. Дополнение по E2E (25.09): game-folder моды нельзя трогать из-под игры

Проверено на живом `Recipe Pin` (3807460866):
- **первичная установка** game-folder мода ограничений не имеет (файлов ещё нет — обновлять/удалять нечего);
- **обновление/удаление** — нельзя: `Managed\0Harmony.dll`, `GK2RecipePin.dll`, `GK2RecipePin.Core.dll` залочены процессом уже до самого раннего патчер-хука (`UnityEngine.CoreModule.dll`, он есть в списке BepInEx). Загрузчик вёл себя fail-closed (бэкап сохранён), но задачу не выполнял;
- **при этом**: подменить файлы нельзя, а в trust результат записывался как успешный, даже если установка не произошла (тогда следующий апдейт теряется).

**Решение (C):**
- **B** — DLL game-folder мода **не перемещать** в папку игры, а **грузить в процессе** из папки айтема (`Assembly.Load(File.ReadAllBytes(...))`, `0Harmony` подтянется из `BepInEx\core`): заблокированные/занятые файлы больше не мешают; копировать только не-DLL файлы (`Languages`, данные) — это их задача.
- **A** — то, что нельзя сделать в игре (удаление legacy-копий DLL из `Managed`), загрузчик пишет в `BepInEx\config\GK2_WorkshopLoader.pendinggame.txt`, а `GK2 Mod Installer` применяет это при закрытой игре.
- Плюс: при неудачной установке/откате не портить trust-чек (иначе обновление теряется).

Итог: код game-folder мода живёт в айтеме и грузится в процессе; в папке игры — только данные. Правки из айтема приезжают без обновления файлов в `Managed`.
